...
void Anunciador::settearGanadores(std::vector<int>& ganadores){
    ...
    int x = 200;
    int y = 300;
    fuentes.addFont("ganadores", "../../resources/fuentes/Nougat.ttf", 35);
    SDL_Color color {255, 0, 0};
    Label ganador(200, 200, "GANADORES", fuentes.getFont("ganadores"), color, ventana->obtener_render());
    ...
    for (int i = 0;i < ganadores.size(); i++){
        ...
        if (jugador->getId() == id){
            ...
            Label label(x,y, informacion, fuentes.getFont("ganadores"), color, ventana->obtener_render());
